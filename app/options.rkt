...
(require racket/gui "../lib/number-field.rkt")
...
(define (options app-tools p-frame textRefresher)
  ...
  (define BAUDS (new number-field% (label "Bauds : ") (parent VPANEL)
                     (init-value (~a (send config bauds)))))
  (define ALSA (new check-box% (label "Utiliser ALSA") (parent VPANEL)
                    (value (send config alsa))))
  (define SAMPLE-RATE (new number-field% (label "Fréquence d'echantillonage : ") (parent VPANEL)
